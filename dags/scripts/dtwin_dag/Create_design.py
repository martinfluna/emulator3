...
for i in range(mbr_list.shape[0]):
    Params_ref_row=(Params_ref*(1+0.00*(np.random.random(len(Params_ref))-.5)/2)).tolist()
    Params[i]=Params_ref_row+(850*(1+0.00*(np.random.random(1)-.5)/2)).tolist()+(90*(1+0.66*(np.random.random(1)-.5)/2)).tolist()
...
for i1 in Exp_list:
    ...
    DTWIN_config[i1]['Pulse_profile']={'time_pulse':time_pulses.tolist(),'Feed_pulse':(0+np.zeros(len(time_pulses.tolist()))).tolist(),
    ...
    DTWIN_config[i1]['time_sample']['DOT']=np.linspace(0.04,t_duration,int(t_duration)*25).tolist()
